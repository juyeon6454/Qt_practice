...
int Widget::loadGLTextures()    //Load Bitmaps And Convert To Textures
{
    ...
    QImage image1("bareel.jpg");

    glGenTextures(1, &texture[0]);
/*    texture[0] = bindTexture(image1);  */ //Create Three Textures
    ...
    //Cread MipMapped Texture
    glBindTexture(GL_TEXTURE_2D, texture[1]);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_NEAREST);
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, image2 -> size().width(), image2->size().height(), GL_BGRA, GL_UNSIGNED_BYTE, data2);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_NEAREST);
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, image3->size().width(), image3->size().height(), GL_BGRA, GL_UNSIGNED_BYTE, data3);

//    QImage* image4 = new QImage("opencom.jpg");
//    GLubyte *data4 = image4 ->bits();
//    glGenTextures(1, &texture[3]);      //Create Three Textures

//    //Crete MipMapped Texture
//    glBindTexture(GL_TEXTURE_2D, texture[3]);
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_NEAREST);
//    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, image4->size().width(), image4->size().height(), GL_RGBA, GL_UNSIGNED_BYTE, data4);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_NEAREST);
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, image5->size().width(), image5->size().height(), GL_RGBA, GL_UNSIGNED_BYTE, data5);
...
void Widget::updateTimer()
{
    ...
    if(enemyAround == FALSE) {
        //set the values for the ennmy portion of the code
        normalEnemy.ypos = 2.5;
        normalEnemy.speed = .02;
        normalEnemy.xpos =  -22 + (QRandomGenerator::global()->generate())%(45);
        normalEnemy.xpos = normalEnemy.xpos*.1;
